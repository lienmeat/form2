...
class Forms extends MY_Controller
{
    ...
    public function viewEmbedded($name)
    {
        $form = $this->form->getPublishedWithName($name);
        if ($form) {
    ...
    public function postForm($id)
    {
        ...
        if (!empty($post) && $post['f2token'] == $_SESSION['f2']['form_token']) {
            ...
            if (!$form) {
                $this->_failAuthResp('That form could not be found!');
            }
    ...
    private function _checkViewRights($form)
    {
        if ($form->config->login_required == 'Y') {
            ...
            if (is_array($form->config->ad_groups)) {
                if (!in_array("all", $form->config->ad_groups)) {
                    //any valid login will do if this fails
                    if (!(count(array_intersect(array_map(strtolower, $this->authorization->memberof()), array_map(strtolower, $form->config->ad_groups))) > 0 ||
                        in_array(strtolower($this->authorization->status()), $form->config->ad_groups))) {
                        ...
                        if (!empty($viewers)) {
                            if (!in_array(strtolower($this->authorization->username()), $viewers)) {
                                if ($can_view) {
                                    ...
                                    $this->_failAuthResp("You must have one of the following AD statuses to view this form:<br />" . implode(", ", $form->config->ad_groups) . "<br />OR be one of these users:<br />" . implode(", ", $viewers));
                                }
                            ...
                            if ($can_view) {
                                ...
                                $this->_failAuthResp("You must have one of the following AD statuses to view this form:<br />" . implode(", ", $form->config->ad_groups));
                            }
                ...
                //no ad_status configured, check viewers
                if (!empty($viewers)) {
                    if (!in_array(strtolower($this->authorization->username()), $viewers)) {
                        if ($can_view) {
                            return;
                        } else {
                            $this->_failAuthResp("You must be one of the following users to access this form:<br />" . implode(", ", $viewers));
                        }
    ...
    public function indenticalPost($form_id)
    {
        $post = $this->input->post(null, true);
    ...
    public function add()
    {
        ...
        if (empty($post)) {
            ...
            $form = new stdClass();
            $form->config = new stdClass();
            $this->load->view('add_form', ['form' => $form]);
        } else {
            $form = $post;
     ...
     */
    public function edit($name_or_id = false)
    {
        $this->authorization->forceLogin();
...
//othwise notify of fail
            echo json_encode(array('status' => 'fail'));
        }
    }

    public function rename($form_name)
    {
        $this->authorization->forceLogin();
     ...
     */
    public function copy($name_or_id)
    {
        $this->authorization->forceLogin();
     ...
     */
    public function results($name_or_id = false)
    {
        $this->authorization->forceLogin();
